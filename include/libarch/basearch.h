...
const char ** libarch_basearch_get_compatible_native_arches(const char * basearch);
...
///
/// Data ownership: The returned list MUST be freed. The individual list items are static and MUST NOT be freed.
const char ** libarch_basearch_get_compatible_multilib_arches(const char * basearch);
